...
// any_cast
TEST(any_tests, test_any_cast){
    int v = 10;
    utils::Any a = v;
    // EXPECT_EQ(utils::any_cast<const int>(&a), 10);
    // EXPECT_EQ(utils::any_cast<int&>(&a), 10);
...
//     ASSERT_THROW(utils::any_cast<Point>(&a), std::runtime_error);
