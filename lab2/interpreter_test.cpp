...
TEST(interpreter_test, push_many_positive_number ){
    ...
    EXPECT_EQ(interpreter.interpret(exp), "< ok\n");  
    EXPECT_EQ(interpreter.pop(), 5);
    EXPECT_EQ(interpreter.pop(), 4);
    EXPECT_EQ(interpreter.pop(), 3);
    EXPECT_EQ(interpreter.pop(), 2);
    EXPECT_EQ(interpreter.pop(), 1);
}
...
TEST(interpreter_test, push_multi_digit_positive_number ){
    ...
    EXPECT_EQ(interpreter.interpret(exp), "< ok\n");  
    EXPECT_EQ(interpreter.pop(), 2147483647);
}
...
TEST(interpreter_test, push_multi_digit_negative_number ){
    ...
    EXPECT_EQ(interpreter.interpret(exp), "< ok\n");  
    EXPECT_EQ(interpreter.pop(), -2147483648);
...
TEST(interpreter_test, Add_multi_digit_numbers){
    ...
    Interpreter interpreter = Interpreter::get_instance();
    std::string res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -132134 + 8321);
    interpreter.clear();

    exp = "-1123 323 + -9231 +";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -1123 + 323 + (-9231));
    interpreter.clear();
}    
...
TEST(interpreter_test, Sub_positive_numbers){
    ...
    Interpreter interpreter = Interpreter::get_instance();
    std::string res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), 0);
    interpreter.clear();

    exp = "1 1 - 2 - 2 -";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), (1 - 1) - 2 - 2 );
}  
...
TEST(interpreter_test, Sub_negative_numbers){
    ...
    exp = "-1 1 - -9 5 - -";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), (-1 - 1) - (-9 - 5));
}    
...
TEST(interpreter_test, Sub_multi_digit_numbers){
    ...
    Interpreter interpreter = Interpreter::get_instance();
    std::string res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -2367 - (-3214));
    interpreter.clear();

    exp = "-231 43 - -9541 -";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -231 - 43 - (-9541));
    interpreter.clear();
}    
...
TEST(interpreter_test, Mul_positive_numbers){
    ...
    exp = "1 1 * 2 * 5 *";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), (1 * 1) * 2 * 5 );
}  
...
TEST(interpreter_test, Mul_negative_numbers){
    ...
    exp = "-1 9 * -7 1 * *";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), (-1 * 9) * (-7 * 1));
}    
...
TEST(interpreter_test, Mul_multi_digit_numbers){
    ...
    Interpreter interpreter = Interpreter::get_instance();
    std::string res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -2367 * 231);
    interpreter.clear();

    exp = "-223 4 * -2341 *";
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -223 * 4 * (-2341));
    interpreter.clear();
}    
...
TEST(interpreter_test, test_swap){
    ...
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -321321);
    EXPECT_EQ(interpreter.pop(), 32133123);
    interpreter.clear();
...
TEST(interpreter_test, test_rot){
    ...
    res = interpreter.interpret(exp);
    EXPECT_EQ(interpreter.pop(), -93212);
    EXPECT_EQ(interpreter.pop(), 32133123);
    EXPECT_EQ(interpreter.pop(), -321321);
    EXPECT_EQ(interpreter.pop(), 23);
    interpreter.clear();
}   
...
TEST(interpreter_test, too_many_spaces){
    ...
    EXPECT_EQ(interpreter.pop(), -312);
    EXPECT_EQ(interpreter.pop(), 238939);
    EXPECT_EQ(interpreter.pop(), 212);
    EXPECT_EQ(interpreter.pop(), -21);
    interpreter.clear();
